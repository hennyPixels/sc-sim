...
## A Dwarven Chronicle of Computation, Construction, and Communication
...
```
    ...
    ║              THE SILMARILLION OF ALGORITHMS                          ║
    ║                                                                      ║
    ║         Being the History of Computation as Practiced by             ║
    ║          the Khazâd in the Deeps of Khazad-dûm and Beyond           ║
    ║                                                                      ║
    ║    ─────────────────────────────────────────────────────────────     ║
    ║                                                                      ║
    ...
    ║    ─────────────────────────────────────────────────────────────     ║
    ║                                                                      ║
    ║    Applied to the PartsDB System of Inventory Management            ║
    ║                                                                      ║
    ╚══════════════════════════════════════════════════════════════════════╝
```
...
### THE AINULINDALË (Part I): The Music of Computation
- I-A: The Music of Computation — Origins of Algorithmic Thought
- I-B: The First Algorithms — Fundamental Operations
- I-C: The Complexity of Arda — Understanding Big-O Notation

### THE VALAQUENTA (Part II): The Powers of Data Structures
- II-A: Arrays and the Halls of Stone
- II-B: Linked Lists and the Chains of Angband
- II-C: Trees and the Two Trees of Valinor

### THE QUENTA KHAZADGARMÂ (Part III): Dwarven Sorting Algorithms
- III-A: Mining Sorts — Bubble Sort and Selection Sort
- III-B: Construction Sorts — Merge Sort and Quick Sort
- III-C: Master Crafts — Heap Sort and Radix Sort

### THE RAILWAYS OF KHAZAD-DÛM (Part IV): Graph Algorithms
- IV-A: Tunnel Networks — Graph Representation
- IV-B: Path Finding — BFS, DFS, and Dijkstra's Algorithm
- IV-C: The Grand Design — Minimum Spanning Trees and Network Flow

### THE PALANTÍR PROTOCOL (Part V): Dwarven Communication
- V-A: Water Power Generation — Hydroelectric Innovation
- V-B: The Iglishmêk Telegraph — Dwarven Morse Code
- V-C: Implementation — Building the Communication Network

### THE NAUGLAMÍR SCAVENGER HUNT (Part VI): Challenges and Quests
- VI-A: The Sedgewick Stones — Algorithm Challenges
- VI-B: The Grokking Gems — Visual Learning Quests
- VI-C: The PartsDB Integration — Practical Application

### APPENDICES
- A: The Khuzdul Algorithm Reference
- B: Morse Code and Iglishmêk Translation Tables
- C: Complete Code Index
...
```
    "In the beginning Ilúvatar made the Ainur of his thought,
     and they made a great Music before him. In this Music
     the World was begun; for Ilúvatar made visible the song
     that they had sung, and they beheld it as a light in the darkness."
...
## Part I-A: The Music of Computation

### Prologue: Before the Forging

In the beginning, before the great forges of Khazad-dûm were lit, before the Dwarves delved their halls of stone, there was the Music—the fundamental harmony that underlies all of creation.

The Dwarves, fashioned by Aulë the Smith, inherited his love of making things. But unlike the other races of Middle-earth, the Khazâd understood that creation was not merely about raw materials and muscle. It required *method*. It required *order*. It required what they called in their secret tongue: **Khazâd-khuzdûl**—the "Algorithms of the Makers."

---

### Chapter 1: The Song of Aulë — What is an Algorithm?

```c
/*
 * THE DEFINITION OF ALGORITHM
 *
 * As Sedgewick writes in "Algorithms in C":
 *   "An algorithm is a finite, definite, effective procedure
 *    for solving a problem."
 *
 * As the Dwarves understood:
 *   "A forge-song is a sequence of hammer-blows,
 *    each following the last in proper order,
 *    transforming raw ore into worked metal."
 *
 * The three properties of an algorithm:
 *   1. FINITE    — It must terminate
 *   2. DEFINITE  — Each step must be precisely defined
 *   3. EFFECTIVE — Each step must be executable
 */

/*
 * THE FIRST ALGORITHM: Euclid's GCD
 *
 * Even before the Dwarves, the ancient mathematicians
 * discovered this algorithm for finding the Greatest Common Divisor.
 * Sedgewick presents this in Chapter 1.
 */
...
int gcd_euclid(int a, int b) {
     ...
     *
     * When dividing mithril ore into equal portions for different forges,
     * we must find the largest portion size that divides evenly.
     *
     * If forge A needs 48 ingots and forge B needs 18 ingots,
     * what is the largest batch size we can smelt?
     */
    while (b != 0) {
        int temp = b;
        b = a % b;      /* The remainder after division */
        a = temp;
    }
...
/*
 * TRACE OF EXECUTION (as recorded in the Forge-Logs):
 *
 *   gcd(48, 18):
 ...
 *     a=6,  b=0  → return 6
 *
 *   "Thus each forge receives batches of 6 ingots:
 *    Forge A: 8 batches, Forge B: 3 batches."
 */
```

---

### Chapter 2: The Seven Fathers — The Foundational Concepts

The Seven Fathers of the Dwarves, awakened by Ilúvatar, each brought a different aspect of algorithmic wisdom:

```c
/*
 * THE SEVEN FOUNDATIONS OF ALGORITHMIC THOUGHT
 *
 * As taught in the Halls of the Seven Fathers:
 */

/*
 * DURIN I: SEQUENCE
 * "Do this, then this, then this."
 * The most basic form of algorithm—sequential execution.
 */
void durins_sequence(void) {
    light_the_forge();          /* Step 1 */
    heat_the_metal();           /* Step 2 */
    shape_the_metal();          /* Step 3 */
    quench_in_water();          /* Step 4 */
    /* Each step follows the previous in exact order */
}
 ...
 */
void durins_selection(Metal *metal) {
    if (metal->type == MITHRIL) {
        use_moonlight_forge();      /* Special handling */
    } else if (metal->type == GOLD) {
        use_standard_forge();
    } else {
        use_common_forge();
    }
 ...
 */
void durins_iteration(Ingot *ingots, int count) {
    for (int i = 0; i < count; i++) {
        hammer_ingot(&ingots[i]);   /* Repeat for each ingot */
    }
}

/*
 * DURIN IV: RECURSION
 * "To solve the whole, first solve the parts."
 * Functions that call themselves.
 */
int durins_recursion(int n) {
    /* Count the rings of a tree (years of growth) */
    if (n <= 1) {
        return 1;               /* Base case: innermost ring */
    }
    return n + durins_recursion(n - 1);  /* Recursive case */
}

/*
 * DURIN V: ABSTRACTION
 * "Name the pattern, use the name."
...
void durins_abstraction(Ore *ore, ForgingProcess *process) {
    Metal *metal = NULL;
    Item *item = NULL;

    process->smelt(ore);        /* Abstract: any smelting method */
    process->shape(metal);      /* Abstract: any shaping method */
    process->finish(item);      /* Abstract: any finishing method */
}

/*
 * DURIN VI: COMPOSITION
 * "Combine simple parts into complex wholes."
 * Building larger algorithms from smaller ones.
 */
Weapon *durins_composition(Ore *ore) {
    Metal *metal = smelt(ore);              /* Sub-algorithm 1 */
    Blade *blade = forge_blade(metal);      /* Sub-algorithm 2 */
    Handle *handle = carve_handle();        /* Sub-algorithm 3 */
    Weapon *weapon = assemble(blade, handle); /* Composition */
    return weapon;
}

/*
 * DURIN VII: TRANSFORMATION
 * "Change the form while preserving the essence."
 * Converting data from one form to another.
 */
void durins_transformation(RawOre *input, RefinedMetal *output) {
    /* Input: Raw ore with impurities
     * Output: Pure metal ready for forging
     * The mass is reduced, but the quality is preserved.
     */
    output->purity = 1.0;
    output->mass = input->mass * input->metal_content;
    output->type = input->primary_metal;
}
```

---

### Chapter 3: The Awakening — Understanding Input and Output

```c
/*
 * THE NATURE OF COMPUTATION
 *
 * Every algorithm transforms INPUT into OUTPUT.
 * This is the fundamental nature of computation.
 *
 * In Grokking Algorithms, Chapter 1:
 *   "An algorithm is a set of instructions for accomplishing a task."
 *
 * The Dwarven understanding:
 *   INPUT:  Raw materials from the mines
 *   PROCESS: The forge-work and crafting
 *   OUTPUT: Finished goods for trade or use
 */

/*
 * THE PARTSDB PARALLEL
 *
 * In our PartsDB system, we see the same pattern:
 */
...
/* THE ALGORITHM: Transform criteria into results */
SearchResults *find_parts(PartsDB *db, SearchCriteria *criteria) {
    /*
     * Like a Dwarven prospector who takes a description
     * of desired ore and returns the locations of matching veins.
     *
     * INPUT:  "I need mithril, at least 10 units, under 1000 gold"
     * OUTPUT: List of matching inventory locations
     */
    SearchResults *results = malloc(sizeof(SearchResults));
    results->results = malloc(db->count * sizeof(Part *));
    results->count = 0;

    clock_t start = clock();

    for (size_t i = 0; i < db->count; i++) {
        if (matches_criteria(db->parts[i], criteria)) {
            results->results[results->count++] = db->parts[i];
        }
    }

    clock_t end = clock();
    results->search_time_ms = (double)(end - start) / CLOCKS_PER_SEC * 1000;
 ...
 * ════════════════════════════════════════════════════════════════
 * In Sedgewick's "Algorithms in C", Part 1, Chapter 1:
 * Find the section where he discusses the difference between
 * "algorithms" and "programs." What metaphor does he use?
 *
 * HINT: It involves a recipe and a meal.
 *
 * Write the page number here when you find it: _______
 * ════════════════════════════════════════════════════════════════
 */
```
...
### Chapter 4: The First Words — Pseudocode and the Secret Tongue

The Dwarves kept their language secret, revealing it to few outsiders. Similarly, algorithms have their own "secret tongue"—pseudocode—a way of expressing ideas without committing to a specific programming language.

```c
/*
 * PSEUDOCODE: THE KHUZDUL OF ALGORITHMS
 *
 * Just as Khuzdul was used among Dwarves to discuss matters
 * they wished to keep private, pseudocode allows us to discuss
 * algorithms without implementation details.
 */

/*
 * PSEUDOCODE EXAMPLE: Binary Search
 *
 * In Grokking Algorithms, Chapter 1, the binary search is
 * described with this visual approach:
 *
 ...
 * THE DWARVEN INTERPRETATION:
 *
 * When searching the Great Ledgers of Khazad-dûm for a specific
 * account, the archivists do not read every page. Instead:
 *
 * 1. Open the ledger to the middle
 * 2. Is the account name before or after this page?
 * 3. If before, look in the first half
 * 4. If after, look in the second half
 * 5. Repeat until found
 *
 * This is why the ledgers are kept in alphabetical order!
 */

int binary_search(Part **parts, int count, const char *part_number) {
    int low = 0;
    int high = count - 1;

    while (low <= high) {
        int mid = low + (high - low) / 2;  /* Avoid overflow */
        int cmp = strcmp(parts[mid]->part_number, part_number);
 ...
 *
 * Linear search: Check every item → O(n) operations
 * Binary search: Halve the search space each time → O(log n) operations
 *
 * For 1,000,000 parts:
 *   Linear search: up to 1,000,000 comparisons
 *   Binary search: at most 20 comparisons (log₂ 1,000,000 ≈ 20)
 *
 * As the Dwarven proverb says:
 *   "A wise miner does not dig every tunnel;
 *    he follows the veins of ore."
 */
```

---

### Chapter 5: The Lamps of the Valar — Correctness and Testing

```c
/*
 * THE THREE LAMPS OF ALGORITHMIC CORRECTNESS
 *
 * Before the Sun and Moon, the Valar created two great Lamps
 * to light Middle-earth. Similarly, three "lamps" illuminate
 * whether an algorithm is correct:
 *
 * LAMP 1: PRECONDITIONS
 *   What must be true BEFORE the algorithm runs?
 *
 * LAMP 2: POSTCONDITIONS
 *   What must be true AFTER the algorithm runs?
 *
 * LAMP 3: INVARIANTS
 *   What must ALWAYS be true during execution?
 */

/*
 * EXAMPLE: Insertion into a sorted array
 */

/* PRECONDITION: Array is sorted in ascending order */
/* POSTCONDITION: Array is still sorted, contains new element */
/* INVARIANT: At each step, processed portion remains sorted */

bool insert_sorted(Part **parts, size_t *count, size_t capacity, Part *new_part) {
    /* Check precondition */
    assert(*count < capacity && "Array must have space");
    assert(is_sorted(parts, *count) && "Array must be sorted");

    /* Find insertion point (maintaining invariant) */
    size_t insert_pos = 0;
    while (insert_pos < *count &&
           strcmp(parts[insert_pos]->part_number, new_part->part_number) < 0) {
        insert_pos++;
        /* INVARIANT: parts[0..insert_pos-1] are all less than new_part */
    }

    /* Shift elements to make room */
    for (size_t i = *count; i > insert_pos; i--) {
        parts[i] = parts[i - 1];
        /* INVARIANT: parts[i..count] are correctly positioned */
    }

    /* Insert the new element */
    parts[insert_pos] = new_part;
    (*count)++;

    /* Check postcondition */
    assert(is_sorted(parts, *count) && "Array must remain sorted");

    return true;
}

/*
 * THE DWARVEN QUALITY ASSURANCE
 *
 * Before any blade leaves the forge, it is tested:
 *   1. Visual inspection (syntax checking)
 *   2. Balance testing (logic verification)
 *   3. Strike testing (runtime testing)
 *   4. Use in battle (production testing)
 *
 * Similarly, we test our algorithms at multiple levels.
 */
...
void test_binary_search(void) {
    ...
    Part *parts[5];
    parts[0] = create_part("A001", "Alpha", 10);
    parts[1] = create_part("B002", "Beta", 20);
    parts[2] = create_part("C003", "Gamma", 30);
    parts[3] = create_part("D004", "Delta", 40);
    parts[4] = create_part("E005", "Epsilon", 50);

    /* Test: Find existing element */
    assert(binary_search(parts, 5, "C003") == 2);
    printf("  ✓ Found existing element at correct position\n");

    /* Test: Find first element */
    assert(binary_search(parts, 5, "A001") == 0);
    printf("  ✓ Found first element\n");

    /* Test: Find last element */
    assert(binary_search(parts, 5, "E005") == 4);
    printf("  ✓ Found last element\n");

    /* Test: Element not found */
    assert(binary_search(parts, 5, "X999") == -1);
    printf("  ✓ Correctly returned -1 for missing element\n");

    /* Test: Empty array */
    assert(binary_search(parts, 0, "A001") == -1);
    printf("  ✓ Correctly handled empty array\n");
 ...
 * ════════════════════════════════════════════════════════════════
 * In Grokking Algorithms, Chapter 1, there is a visual illustration
 * of binary search using a guessing game with numbers 1-100.
 *
 * How many guesses does it take (maximum) to find any number
 * from 1 to 100 using binary search?
 *
 * ANSWER: _______
 *
 * Now calculate: For a phonebook with 1,000,000 names,
 * how many maximum comparisons would binary search need?
 *
 * ANSWER: _______
 * ════════════════════════════════════════════════════════════════
 */
...
║     • Finite, definite, effective procedure                            ║
║     • Transforms input into output                                     ║
║     • Like a Dwarven forge-song guiding craftsmanship                 ║
║                                                                        ║
║  2. THE SEVEN FOUNDATIONS                                              ║
║     • Sequence    — Steps in order                                     ║
║     • Selection   — If/then decisions                                  ║
║     • Iteration   — Loops and repetition                               ║
║     • Recursion   — Self-referential solutions                         ║
║     • Abstraction — Naming patterns                                    ║
║     • Composition — Building complex from simple                       ║
║     • Transformation — Converting data forms                           ║
║                                                                        ║
║  3. BINARY SEARCH                                                      ║
